...
CREATE INDEX folder_shares_workspace_id_idx ON folder_shares USING btree (workspace_id);
CREATE INDEX folder_shares_share_token_idx ON folder_shares USING btree (share_token);
CREATE INDEX folder_shares_shared_by_idx ON folder_shares USING btree (shared_by);
CREATE INDEX folder_shares_expires_at_idx ON folder_shares USING btree (expires_at);
CREATE INDEX folder_shares_is_active_idx ON folder_shares USING btree (is_active);

-- RLS for folder_shares
ALTER TABLE folder_shares ENABLE ROW LEVEL SECURITY;

-- Policy for selecting data - allow anonymous access to valid shares
...
-- Function to increment access count when share is accessed
